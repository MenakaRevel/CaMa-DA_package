...
def stats(inputlist):
  ...
  # sfcelv
  fname=os.path.join('./CaMa_out',input_name,'sfcelv'+tag+'.nc')
  with xr.open_dataset(fname) as ds:
      start_dt=datetime.date(syear,1,1)#'%02d-%02d-%04d'%(1,1,syear)
      end_dt=datetime.date(eyear,12,31)#'%02d-%02d-%04d'%(31,12,eyear)
      da = ds['sfcelv'].sel(time=slice(start_dt,end_dt)).load()
      sfcelv_mean=da.mean(dim='time').values
      sfcelv_std=da.std(dim='time').values
      sfcelv_mean.tofile(os.path.join('./CaMa_out',input_name,'sfcelv_mean'+tagout+'.bin'))
      sfcelv_std.tofile(os.path.join('./CaMa_out',input_name,'sfcelv_std'+tagout+'.bin'))
      print (input_name,syear,eyear,np.shape(sfcelv_mean))
  return 0
#-----
